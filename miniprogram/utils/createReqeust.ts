...
export function createRequest({
  requestInterceptor: outerRequest,
  responseInterceptor: outerResponse,
  errorInterceptor: outerError,
  ...
  function Request<responseType>({
    ...
    const handler: requestTask<responseType> = new Promise(
      (resolve, reject) => {
        // Object.assign() 第一个参数为 {} 让后续的参数合并到这个对象字面量上
        // 使得每次创建的对象都唯一
        const option = requestInterceptor
